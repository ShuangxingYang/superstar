...
1. **每日日志 `memory/YYYY-MM-DD.md`(只追加)** —— 成本最低、收益明显。让 Agent 记「今天做了啥、观察到啥」,开会话加载今天+昨天。这一层是引出后面检索/衰减的前提。
   - 接入点:我们没有 `/new`/`/reset`,最自然是在 **`session_store.create()` 新建会话时**加载,或每会话开始定一次(**别每轮读**,日志会随对话变)。
   - 滚动窗口:本地单用户日志增长慢,「今天+昨天」或「最近 N 条」都行,不必照搬。
2. **压缩前记忆刷新** —— **前提是先有 compaction(M12)**。顺序上应等上下文裁剪做了再说。
3. **混合检索(BM25 + 向量)** —— 只有当记忆多到「全量注入撑爆 context」时才需要。我们**已有 RAG 基建(Qdrant)可复用**,不必像 OpenClaw 引 SQLite;但要补 BM25 关键词腿才算「混合」。
4. **时间衰减** —— 最后考虑,且依赖①先有按日期堆积的记忆。
